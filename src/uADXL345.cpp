...
#define ADXL345_DEVICE (0x53)    // Device Address for uADXL345
...
void uADXL345::readAccel(int *xyz){
	readAccel(xyz, xyz + 1, xyz + 2);
}

void uADXL345::readAccel(int *x, int *y, int *z) {
	readFrom(ADXL345_DATAX0, ADXL345_TO_READ, _buff);	// Read Accel Data from uADXL345
...
void uADXL345::get_Gxyz(double *xyz){
	int i;
	int xyz_int[3];
	readAccel(xyz_int);
	for(i=0; i<3; i++){
...
void uADXL345::getRangeSetting(byte* rangeSetting) {
	byte _b;
	readFrom(ADXL345_DATA_FORMAT, 1, &_b);
	*rangeSetting = _b & B00000011;
}

void uADXL345::setRangeSetting(int val) {
	byte _s;
	byte _b;
...
void uADXL345::setSelfTestBit(bool selfTestBit) {
	setRegisterBit(ADXL345_DATA_FORMAT, 7, selfTestBit);
}

/*********************** INT_INVERT BIT STATE ***********************/
/*                           ~ GET & SET                            */
bool uADXL345::getInterruptLevelBit() {
	return getRegisterBit(ADXL345_DATA_FORMAT, 5);
}

// If Set (0) Sets the Interrupts to Active HIGH
// If Set (1) Sets the Interrupts to Active LOW
void uADXL345::setInterruptLevelBit(bool interruptLevelBit) {
	setRegisterBit(ADXL345_DATA_FORMAT, 5, interruptLevelBit);
}

/************************* FULL_RES BIT STATE ***********************/
/*                           ~ GET & SET                            */
bool uADXL345::getFullResBit() {
...
void uADXL345::setTapThreshold(int tapThreshold) {
...
/*                           ~ SET & GET                            */
void uADXL345::setAxisGains(double *_gains){
	int i;
	for(i = 0; i < 3; i++){
		gains[i] = _gains[i];
	}
}
void uADXL345::getAxisGains(double *_gains){
	int i;
	for(i = 0; i < 3; i++){
		_gains[i] = gains[i];
	}
...
void uADXL345::setAxisOffset(int x, int y, int z) {
	writeTo(ADXL345_OFSX, byte (x));
	writeTo(ADXL345_OFSY, byte (y));
	writeTo(ADXL345_OFSZ, byte (z));
}

void uADXL345::getAxisOffset(int* x, int* y, int*z) {
	byte _b;
...
void uADXL345::setTapDuration(int tapDuration) {
	tapDuration = constrain(tapDuration,0,255);
	byte _b = byte (tapDuration);
	writeTo(ADXL345_DUR, _b);
}

int uADXL345::getTapDuration() {
	byte _b;
...
void uADXL345::setDoubleTapLatency(int doubleTapLatency) {
	byte _b = byte (doubleTapLatency);
	writeTo(ADXL345_LATENT, _b);
}

int uADXL345::getDoubleTapLatency() {
	byte _b;
...
// It Accepts a Maximum Value of 255.
void uADXL345::setActivityThreshold(int activityThreshold) {
...
int uADXL345::getActivityThreshold() {
	byte _b;
...
// It Accepts a Maximum Value of 255.
void uADXL345::setInactivityThreshold(int inactivityThreshold) {
	inactivityThreshold = constrain(inactivityThreshold,0,255);
	byte _b = byte (inactivityThreshold);
	writeTo(ADXL345_THRESH_INACT, _b);
}

int uADXL345::getInactivityThreshold() {
	byte _b;
...
void uADXL345::setTimeInactivity(int timeInactivity) {
	timeInactivity = constrain(timeInactivity,0,255);
	byte _b = byte (timeInactivity);
	writeTo(ADXL345_TIME_INACT, _b);
}

int uADXL345::getTimeInactivity() {
	byte _b;
...
// Value of 0 May Result in Undesirable Behavior if the Free-Fall Interrupt Enabled.
// Accepts a Maximum Value of 255.
void uADXL345::setFreeFallDuration(int freeFallDuration) {
	freeFallDuration = constrain(freeFallDuration,0,255);
	byte _b = byte (freeFallDuration);
	writeTo(ADXL345_TIME_FF, _b);
}

int uADXL345::getFreeFallDuration() {
	byte _b;
...
/************************** ACTIVITY BITS ***************************/
/*                                                                  */
bool uADXL345::isActivityXEnabled() {
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 6);
}
bool uADXL345::isActivityYEnabled() {
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 5);
}
bool uADXL345::isActivityZEnabled() {
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 4);
}
bool uADXL345::isInactivityXEnabled() {
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 2);
}
bool uADXL345::isInactivityYEnabled() {
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 1);
}
bool uADXL345::isInactivityZEnabled() {
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 0);
}

void uADXL345::setActivityX(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 6, state);
}
void uADXL345::setActivityY(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 5, state);
}
void uADXL345::setActivityZ(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 4, state);
}
void uADXL345::setActivityXYZ(bool stateX, bool stateY, bool stateZ) {
	setActivityX(stateX);
	setActivityY(stateY);
	setActivityZ(stateZ);
}
void uADXL345::setInactivityX(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 2, state);
}
void uADXL345::setInactivityY(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 1, state);
}
void uADXL345::setInactivityZ(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 0, state);
}
void uADXL345::setInactivityXYZ(bool stateX, bool stateY, bool stateZ) {
	setInactivityX(stateX);
	setInactivityY(stateY);
	setInactivityZ(stateZ);
}

bool uADXL345::isActivityAc() {
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 7);
}
bool uADXL345::isInactivityAc(){
	return getRegisterBit(ADXL345_ACT_INACT_CTL, 3);
}

void uADXL345::setActivityAc(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 7, state);
}
void uADXL345::setInactivityAc(bool state) {
	setRegisterBit(ADXL345_ACT_INACT_CTL, 3, state);
}

/************************* SUPPRESS BITS ****************************/
/*                                                                  */
bool uADXL345::getSuppressBit(){
	return getRegisterBit(ADXL345_TAP_AXES, 3);
}
void uADXL345::setSuppressBit(bool state) {
	setRegisterBit(ADXL345_TAP_AXES, 3, state);
}

/**************************** TAP BITS ******************************/
/*                                                                  */
bool uADXL345::isTapDetectionOnX(){
	return getRegisterBit(ADXL345_TAP_AXES, 2);
}
void uADXL345::setTapDetectionOnX(bool state) {
	setRegisterBit(ADXL345_TAP_AXES, 2, state);
}
bool uADXL345::isTapDetectionOnY(){
	return getRegisterBit(ADXL345_TAP_AXES, 1);
}
void uADXL345::setTapDetectionOnY(bool state) {
	setRegisterBit(ADXL345_TAP_AXES, 1, state);
}
bool uADXL345::isTapDetectionOnZ(){
	return getRegisterBit(ADXL345_TAP_AXES, 0);
}
void uADXL345::setTapDetectionOnZ(bool state) {
	setRegisterBit(ADXL345_TAP_AXES, 0, state);
}

void uADXL345::setTapDetectionOnXYZ(bool stateX, bool stateY, bool stateZ) {
	setTapDetectionOnX(stateX);
	setTapDetectionOnY(stateY);
	setTapDetectionOnZ(stateZ);
}

bool uADXL345::isActivitySourceOnX(){
	return getRegisterBit(ADXL345_ACT_TAP_STATUS, 6);
}
bool uADXL345::isActivitySourceOnY(){
	return getRegisterBit(ADXL345_ACT_TAP_STATUS, 5);
}
bool uADXL345::isActivitySourceOnZ(){
	return getRegisterBit(ADXL345_ACT_TAP_STATUS, 4);
}

bool uADXL345::isTapSourceOnX(){
	return getRegisterBit(ADXL345_ACT_TAP_STATUS, 2);
}
bool uADXL345::isTapSourceOnY(){
	return getRegisterBit(ADXL345_ACT_TAP_STATUS, 1);
}
bool uADXL345::isTapSourceOnZ(){
	return getRegisterBit(ADXL345_ACT_TAP_STATUS, 0);
}

/*************************** ASLEEP BIT *****************************/
/*                                                                  */
bool uADXL345::isAsleep(){
	return getRegisterBit(ADXL345_ACT_TAP_STATUS, 3);
}

/************************** LOW POWER BIT ***************************/
/*                                                                  */
bool uADXL345::isLowPower(){
	return getRegisterBit(ADXL345_BW_RATE, 4);
}
void uADXL345::setLowPower(bool state) {
	setRegisterBit(ADXL345_BW_RATE, 4, state);
}

/*************************** RATE BITS ******************************/
/*                                                                  */
double uADXL345::getRate(){
	byte _b;
...
void uADXL345::setRate(double rate){
	byte _b,_s;
	int v = (int) (rate / 6.25);
	int r = 0;
	while (v >>= 1)
...
/*************************** BANDWIDTH ******************************/
/*                          ~ SET & GET                             */
void uADXL345::set_bw(byte bw_code){
...
bool uADXL345::triggered(byte interrupts, int mask){
...
byte uADXL345::getInterruptSource() {
	byte _b;
	readFrom(ADXL345_INT_SOURCE, 1, &_b);
	return _b;
}

bool uADXL345::getInterruptSource(byte interruptBit) {
	return getRegisterBit(ADXL345_INT_SOURCE,interruptBit);
}

bool uADXL345::getInterruptMapping(byte interruptBit) {
...
void uADXL345::setInterruptMapping(byte interruptBit, bool interruptPin) {
	setRegisterBit(ADXL345_INT_MAP, interruptBit, interruptPin);
}

void uADXL345::setImportantInterruptMapping(int single_tap, int double_tap, int free_fall, int activity, int inactivity) {
	if(single_tap == 1) {
...
bool uADXL345::isInterruptEnabled(byte interruptBit) {
	return getRegisterBit(ADXL345_INT_ENABLE,interruptBit);
}

void uADXL345::setInterrupt(byte interruptBit, bool state) {
	setRegisterBit(ADXL345_INT_ENABLE, interruptBit, state);
}

void uADXL345::singleTapINT(bool status) {
	if(status) {
...
void uADXL345::doubleTapINT(bool status) {
	if(status) {
...
void uADXL345::FreeFallINT(bool status) {
	if(status) {
...
void uADXL345::ActivityINT(bool status) {
	if(status) {
...
void uADXL345::InactivityINT(bool status) {
	if(status) {
...
void uADXL345::writeTo(byte _address, byte _val) {
	TinyWireM.beginTransmission(ADXL345_DEVICE);
	TinyWireM.write(_address);
...
void uADXL345::readFrom(byte address, int num, byte _buff[]) {
	TinyWireM.beginTransmission(ADXL345_DEVICE);
	TinyWireM.write(address);
	TinyWireM.endTransmission();

	//TinyWireM.beginTransmission(ADXL345_DEVICE);
	TinyWireM.requestFrom(ADXL345_DEVICE, num);  // Request 6 Bytes

	int i = 0;
	while(TinyWireM.available())
	{
		_buff[i] = TinyWireM.read();				// Receive Byte
		i++;
	}
	if(i != num){
...
void uADXL345::setRegisterBit(byte regAdress, int bitPos, bool state) {
	byte _b;
...
bool uADXL345::getRegisterBit(byte regAdress, int bitPos) {
	byte _b;
	readFrom(regAdress, 1, &_b);
	return ((_b >> bitPos) & 1);
}
...
void uADXL345::printAllRegister() {
	byte _b;
	Serial.print("0x00: ");
	readFrom(0x00, 1, &_b);
	print_byte(_b);
